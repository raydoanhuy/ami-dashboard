      ...
      {projects.map((project, index) => (
        <Card key={index} className="rounded-2xl shadow-md">
          <CardContent className="p-4 space-y-2">
            ...
            <p><strong>COD:</strong> {project.cod}</p>
            <p><strong>Nghiệm thu:</strong> {project.acceptance}</p>
            <p><strong>FIT kỳ vọng:</strong> {project.fit}</p>
            <p>
              <strong>Rủi ro:</strong>{" "}
              <Badge variant={project.risk === "Cao" ? "destructive" : "default"}>
                {project.risk}
              </Badge>
            </p>
            <p><strong>Khuyến nghị:</strong> {project.recommendation}</p>
          </CardContent>
        </Card>
      ))}
    </div>
